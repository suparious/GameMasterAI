...
def main():
  ...
  #while True:
  save_shot("start", full_screen)
  while bagWeight < 90:
    currentFoward += (time.time() - startTime)
    # Execute fucntions in a logical loop
    checkHealth()
    inventoryCheck()
    unstuck()
    rotate()
    ...
    # Clean garbage
    gc.collect()
  print("Possible bag full, stopping. Weight:", bagWeight,"%")
  closeMenu()
  pyautogui.press(config.weaponActivate)
  time.sleep(.3)
  if not stopped:
    pydirectinput.press(config.autorunKey)
    stopped = True
  time.sleep(.3)
  pyautogui.press(config.prone)
  time.sleep(.3)
  # Clean garbage
  gc.collect()
